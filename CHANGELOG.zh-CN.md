...
### 新增
- **Cursor 账号支持 Comeback 优惠识别**：FREE 账号刷新配额时会查询 welcome-back 资格；有资格时 FREE 徽标改用可恢复套餐颜色，并提供支付链接按钮和「只看有 Comeback 的账号」筛选。

### 修复
- **Cursor 活跃会话撤销会带上官方要求的 session type**：只传 `sessionId` 时接口返回 200 但不会真正踢掉设备；现在会附带 `SESSION_TYPE_WEB` / `SESSION_TYPE_CLIENT`，并在未返回 `success: true` 时提示失败。
- **自动备份不再依赖主窗口保持打开**：调度改到后端进程，应用启动后约 20 秒检查一次，之后每 15 分钟再检查；最小化到托盘后也会继续备份。单个平台导出失败会跳过该平台，不再让整次备份中断。
- **Cursor 备份和转移 JSON 会保留账号标签**：精简导出始终写入 `tags` 字段，再导入时会套用这些标签，而不是丢掉。
